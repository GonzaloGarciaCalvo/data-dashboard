...
export function KPIGrid() {
    ...
    return (
      <Card className="p-6 text-center bg-white dark:bg-black border-slate-200 dark:border-slate-800">
        <p className="text-slate-600 dark:text-slate-400">
          No data loaded. Upload CSV files to see KPIs.
        </p>
      </Card>
    );
  }
  ...
  const filteredKpis = showMV
    ? kpis
    : kpis.filter((k) => k.name !== "Monthly Variation");
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
      {filteredKpis.map((kpi) => {
        ...
        const colorClass =
          colorMap[kpi.name] || "text-slate-500 dark:text-slate-400";
          ...
          >
            ...
            <CardContent>
              <div
                ...
                title={kpi.formattedValue}
              >
                {kpi.formattedValue}
              </div>
              {kpi.variation !== undefined && (
                <p
                  className={`text-xs mt-1 truncate ${kpi.variation >= 0 ? "text-green-600 dark:text-green-400" : "text-red-600 dark:text-red-400"}`}
                >
                  {kpi.variation >= 0 ? "+" : ""}
                  {kpi.variation}% vs previous period
                </p>
              )}
            </CardContent>
          </Card>
