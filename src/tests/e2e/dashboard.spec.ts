...
test.describe("Dashboard Page", () => {
  ...
  test("should load the initial state with welcome message", async ({
    page,
  }) => {
    await expect(page.getByText("Welcome to Data Dashboard")).toBeVisible();
    await expect(
      page.getByText("Start by loading your CSV files"),
    ).toBeVisible();
  });
  ...
  test("should not display KPI section when data is not loaded", async ({
    page,
  }) => {
...
test.describe("CSV Upload", () => {
  ...
  test("should accept file through file input", async ({ page }) => {
    ...
    await expect(
      page.locator('[data-testid="sales-by-customer"]'),
    ).toBeVisible();
    await expect(page.locator('[data-testid="sales-by-date"]')).toBeVisible();
    await expect(
      page.locator('[data-testid="sales-by-product"]'),
    ).toBeVisible();
  });

  test("should accept file through file input after deleted a prvious one", async ({
    page,
  }) => {
    // First upload
    const fileInput = page.locator('input[type="file"]');
    ...
    await expect(page.getByText("KPIs", { exact: true })).not.toBeVisible({
      timeout: 5000,
    });
    await expect(
      page.locator('[data-testid="sales-by-customer"]'),
    ).not.toBeVisible({ timeout: 5000 });
    await expect(page.locator('[data-testid="sales-by-date"]')).not.toBeVisible(
      { timeout: 5000 },
    );
    await expect(
      page.locator('[data-testid="sales-by-product"]'),
