...
export function calculateKPIs(sales: Sale[], times: Time[]): CalculatedKPI[] {
  ...
  const costOfSalePercentage =
    totalSales > 0 ? (totalCosts / totalSales) * 100 : 0;
    ...
    },
    {
      ...
      formattedValue:
        monthlyVariation === null
          ? "N/A"
          : `${monthlyVariation >= 0 ? "+" : ""}${formatPercentage(Math.abs(monthlyVariation ?? 0)).replace("%", "")}%`,
      type: "percentage",
    },
    {
      name: "Units Sold",
...
function getGroupKey(dateStr: string, grouping: ChartGrouping): string {
  ...
  if (grouping === "week") {
    ...
    const pasteDaysOfYear =
      (date.getTime() - firstDayOfYear.getTime()) / 86400000;
    const weekNum = Math.ceil(
      (pasteDaysOfYear + firstDayOfYear.getDay() + 1) / 7,
    );
...
export function getSalesByCustomerChartData(
  sales: Sale[],
  customers: Customer[],
...
export function getSalesByProductChartData(
  sales: Sale[],
  products: Product[],
): { name: string; value: number }[] {
